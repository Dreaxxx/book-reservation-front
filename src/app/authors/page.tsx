        ...
        <main style={{ padding: 24, maxWidth: 860, margin: '0 auto' }}>
            ...
            <section style={{ marginTop: 24, marginBottom: 32 }}>
                ...
                <form onSubmit={submitAuthor} style={{ display: 'grid', gap: 12, maxWidth: 480, marginTop: 8 }}>
                    ...
                    <input placeholder="Nom et prénom" value={authorName} onChange={(e) => setAuthorName(e.target.value)} required />
                    ...
                    {authors.map((a) => (
                        <li key={a.id} style={{ border: '1px solid #ddd', borderRadius: 8, padding: 12 }}>
                            {editId === a.id ? (
                                ...
                                <>
                                    <div>Auteur : {a.name}</div>
                                    <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                                        <button type="button" onClick={() => startEdit(a)}>Modifier</button>
                                        <button type="button" onClick={() => deleteAuthor(a.id)}>Supprimer</button>
                                    </div>
                                </>
                            )}
                        </li>
