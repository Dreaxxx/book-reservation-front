  ...
  return (
    <main style={{ padding: 24, maxWidth: 860, margin: "0 auto" }}>
      ...
      <section style={{ marginTop: 24, marginBottom: 32 }}>
        ...
        <form onSubmit={submit} style={{ display: "grid", gap: 12, maxWidth: 480, marginTop: 8 }}>
      ...
      <section style={{ marginTop: 24, marginBottom: 32 }}>
        ...
        <form onSubmit={submit} style={{ display: "grid", gap: 12, maxWidth: 480, marginTop: 8 }}>
          <label style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 8 }}>Nom de l'auteur : </label>
          <input placeholder="Nom et prénom" value={title} onChange={(e) => setAuthorName(e.target.value)} required />
          <button type="submit">Créer</button>
        </form>
      </section>
      ...
      <section style={{ marginTop: 24, marginBottom: 32 }}>
        ...
        <form onSubmit={submit} style={{ display: "grid", gap: 12, maxWidth: 480 }}>
          ...
          <input required id="" test-id="dueDate" placeholder="Date de retour souhaité" type="date" />
          <button type="submit">Créer</button>
        </form>
      </section>
        ...
        <ul style={{ display: "grid", gap: 12, listStyle: "none", padding: 0 }}>
          {books.map((b) => (
            <li key={b.id} style={{ border: "1px solid #ddd", borderRadius: 8, padding: 12 }}>
              <div style={{ fontWeight: 600 }}>{b.title} {b.year ? `(${b.year})` : ""}</div>
